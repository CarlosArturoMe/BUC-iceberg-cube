    ...
    while d < 3:
        ...
        for i in range(C):
            ...
            if c >= min_sup:
                ...
                if d == 0:
                    key = dimensions[d][i]
                    last[d] = dimensions[d][i]
                elif d == 1:
                    key = last[0] + dimensions[d][i]
                    last[d] = dimensions[d][i]
                elif d == 2:
                    key = last[0] + last[1] + dimensions[d][i]
                    last[d] = dimensions[d][i]
                summary.append((key,str(c)))
...
#dict_sum = {}
#for tup in summary:
#    if tup[0] in dict_sum:
#        dict_sum[tup[0]] = int(dict_sum[tup[0]]) + int(tup[1])
#    else:
#        dict_sum[tup[0]] = int(tup[1])
#print(dict_sum)
